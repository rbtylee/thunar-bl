...
X_LIBS = 
X_PRE_LIBS =  -lSM -lICE
abs_builddir = /home/bodhi/Code/thunar-1.8.14
abs_srcdir = /home/bodhi/Code/thunar-1.8.14
...
install-thunar_scriptsSCRIPTS: $(thunar_scripts_SCRIPTS)
	@$(NORMAL_INSTALL)
	@list='$(thunar_scripts_SCRIPTS)'; test -n "$(thunar_scriptsdir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(thunar_scriptsdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(thunar_scriptsdir)" || exit 1; \
	fi; \
	for p in $$list; do \
	...
	while read type dir files; do \
	     ...
	     test -z "$$files" || { \
	       echo " $(INSTALL_SCRIPT) $$files '$(DESTDIR)$(thunar_scriptsdir)$$dir'"; \
	       $(INSTALL_SCRIPT) $$files "$(DESTDIR)$(thunar_scriptsdir)$$dir" || exit $$?; \
	     } \
	; done

uninstall-thunar_scriptsSCRIPTS:
	@$(NORMAL_UNINSTALL)
	@list='$(thunar_scripts_SCRIPTS)'; test -n "$(thunar_scriptsdir)" || exit 0; \
	files=`for p in $$list; do echo "$$p"; done | \
	       sed -e 's,.*/,,;$(transform)'`; \
	dir='$(DESTDIR)$(thunar_scriptsdir)'; $(am__uninstall_files_from_dir)
...
installdirs-am:
	for dir in "$(DESTDIR)$(thunar_scriptsdir)" "$(DESTDIR)$(appdatadir)" "$(DESTDIR)$(desktopdir)" "$(DESTDIR)$(polkit_policydir)" "$(DESTDIR)$(servicedir)" "$(DESTDIR)$(systemd_userdir)"; do \
...
install-data-am: install-appdataDATA install-desktopDATA \
	...
	install-systemd_userDATA install-thunar_scriptsSCRIPTS
...
uninstall-am: uninstall-appdataDATA uninstall-desktopDATA \
	...
	uninstall-systemd_userDATA uninstall-thunar_scriptsSCRIPTS
...
.PHONY: $(am__recursive_targets) CTAGS GTAGS TAGS all all-am \
	...
	uninstall-desktopDATA uninstall-polkit_policyDATA \
